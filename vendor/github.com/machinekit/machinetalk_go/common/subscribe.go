...
func NewSubscribe(Debuglevel int, Debugname string) *Subscribe {
	tmp := &Subscribe{}
...
func (self *Subscribe) OnFsm_down(e *fsm.Event) {
...
func (self *Subscribe) OnFsm_start(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *Subscribe) OnFsm_trying(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *Subscribe) OnFsm_full_update_received(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *Subscribe) OnFsm_stop(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *Subscribe) OnFsm_up(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *Subscribe) OnFsm_heartbeat_timeout(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *Subscribe) OnFsm_heartbeat_tick(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *Subscribe) OnFsm_any_msg_received(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *Subscribe) ErrorString() string {
	return self.errorString
}

func (self *Subscribe) SetErrorString(es string) {
	if self.errorString != "" {
		return
	}
	self.errorString = es
...
func (self *Subscribe) AddSocketTopic(name string) {
	self.SocketTopics[name] = true
}

func (self *Subscribe) RemoveSocketTopic(name string) {
	delete(self.SocketTopics, name)
}

func (self *Subscribe) ClearSocketTopics() {
	self.SocketTopics = make(map[string]bool)
}

func (self *Subscribe) socketWorker(context *zmq.Context, uri string) {
	poll := zmq.NewPoller()
...
func (self *Subscribe) StartSocket() {
	go self.socketWorker(self.context, self.SocketUri)
}

func (self *Subscribe) StopSocket() {
	self.shutdown.Send(" ", 0) // trigger socket thread shutdown
}

func (self *Subscribe) HeartbeatTimerTick() {
...
func (self *Subscribe) ResetHeartbeatLiveness() {
	self.HeartbeatLiveness = self.HeartbeatResetLiveness
}

func (self *Subscribe) ResetHeartbeatTimer() {
...
func (self *Subscribe) StartHeartbeatTimer() {
	self.HeartbeatActive = true
	self.ResetHeartbeatTimer()
}

func (self *Subscribe) StopHeartbeatTimer() {
...
func (self *Subscribe) SocketMsgReceived(socket *zmq.Socket) {
	...
	if *rx.Type == pb.ContainerType_MT_PING {
		...
		// INCOMING full update 0 1 0

		// react to full update message
	} else if *rx.Type == pb.ContainerType_MT_FULL_UPDATE {
		if rx.Pparams != nil {
		...
		// INCOMING incremental update 0 0 0
	} //BBBBBBBB incremental update
