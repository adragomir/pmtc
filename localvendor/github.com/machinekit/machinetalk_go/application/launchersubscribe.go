...
func NewLauncherSubscribe(Debuglevel int, Debugname string) *LauncherSubscribe {
	tmp := &LauncherSubscribe{}
...
func (self *LauncherSubscribe) OnFsm_down(e *fsm.Event) {
...
func (self *LauncherSubscribe) OnFsm_start(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *LauncherSubscribe) OnFsm_trying(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *LauncherSubscribe) OnFsm_full_update_received(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *LauncherSubscribe) OnFsm_stop(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *LauncherSubscribe) OnFsm_up(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *LauncherSubscribe) OnFsm_heartbeat_timeout(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *LauncherSubscribe) OnFsm_heartbeat_tick(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *LauncherSubscribe) OnFsm_any_msg_received(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *LauncherSubscribe) ErrorString() string {
	return self.errorString
}

func (self *LauncherSubscribe) SetErrorString(es string) {
	if self.errorString != "" {
		return
	}
	self.errorString = es
...
// trigger
func (self *LauncherSubscribe) Start() {
	if self.fsm.Is("down") {
...
// trigger
func (self *LauncherSubscribe) Stop() {
...
func (self *LauncherSubscribe) AddSocketTopic(name string) {
	self.SocketTopics[name] = true
}

func (self *LauncherSubscribe) RemoveSocketTopic(name string) {
	delete(self.SocketTopics, name)
}

func (self *LauncherSubscribe) ClearSocketTopics() {
	self.SocketTopics = make(map[string]bool)
}

func (self *LauncherSubscribe) socketWorker(context *zmq.Context, uri string) {
	poll := zmq.NewPoller()
...
func (self *LauncherSubscribe) StartSocket() {
	go self.socketWorker(self.context, self.SocketUri)
}

func (self *LauncherSubscribe) StopSocket() {
	self.shutdown.Send(" ", 0) // trigger socket thread shutdown
}

func (self *LauncherSubscribe) HeartbeatTimerTick() {
...
func (self *LauncherSubscribe) ResetHeartbeatLiveness() {
	self.HeartbeatLiveness = self.HeartbeatResetLiveness
}

func (self *LauncherSubscribe) ResetHeartbeatTimer() {
...
func (self *LauncherSubscribe) StartHeartbeatTimer() {
	self.HeartbeatActive = true
	self.ResetHeartbeatTimer()
}

func (self *LauncherSubscribe) StopHeartbeatTimer() {
...
func (self *LauncherSubscribe) SocketMsgReceived(socket *zmq.Socket) {
	...
	if *rx.Type == pb.ContainerType_MT_PING {
		...
		// INCOMING launcher full update 0 1 0

		// react to launcher full update message
	} else if *rx.Type == pb.ContainerType_MT_LAUNCHER_FULL_UPDATE {
		if rx.Pparams != nil {
		...
		// INCOMING launcher incremental update 0 0 0
	} //BBBBBBBB launcher incremental update
