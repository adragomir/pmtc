...
func NewStatusSubscribe(Debuglevel int, Debugname string) *StatusSubscribe {
	tmp := &StatusSubscribe{}
...
func (self *StatusSubscribe) OnFsm_down(e *fsm.Event) {
...
func (self *StatusSubscribe) OnFsm_start(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *StatusSubscribe) OnFsm_trying(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *StatusSubscribe) OnFsm_full_update_received(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *StatusSubscribe) OnFsm_stop(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *StatusSubscribe) OnFsm_up(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *StatusSubscribe) OnFsm_heartbeat_timeout(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *StatusSubscribe) OnFsm_heartbeat_tick(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *StatusSubscribe) OnFsm_any_msg_received(e *fsm.Event) {
	if self.Debuglevel > 0 {
...
func (self *StatusSubscribe) ErrorString() string {
	return self.errorString
}

func (self *StatusSubscribe) SetErrorString(es string) {
	if self.errorString != "" {
		return
	}
	self.errorString = es
...
// trigger
func (self *StatusSubscribe) Start() {
	if self.fsm.Is("down") {
...
// trigger
func (self *StatusSubscribe) Stop() {
...
func (self *StatusSubscribe) AddSocketTopic(name string) {
	self.SocketTopics[name] = true
}

func (self *StatusSubscribe) RemoveSocketTopic(name string) {
	delete(self.SocketTopics, name)
}

func (self *StatusSubscribe) ClearSocketTopics() {
	self.SocketTopics = make(map[string]bool)
}

func (self *StatusSubscribe) socketWorker(context *zmq.Context, uri string) {
	poll := zmq.NewPoller()
...
func (self *StatusSubscribe) StartSocket() {
	go self.socketWorker(self.context, self.SocketUri)
}

func (self *StatusSubscribe) StopSocket() {
	self.shutdown.Send(" ", 0) // trigger socket thread shutdown
}

func (self *StatusSubscribe) HeartbeatTimerTick() {
...
func (self *StatusSubscribe) ResetHeartbeatLiveness() {
	self.HeartbeatLiveness = self.HeartbeatResetLiveness
}

func (self *StatusSubscribe) ResetHeartbeatTimer() {
...
func (self *StatusSubscribe) StartHeartbeatTimer() {
	self.HeartbeatActive = true
	self.ResetHeartbeatTimer()
}

func (self *StatusSubscribe) StopHeartbeatTimer() {
...
func (self *StatusSubscribe) SocketMsgReceived(socket *zmq.Socket) {
	...
	if *rx.Type == pb.ContainerType_MT_PING {
		...
		// INCOMING emcstat full update 0 1 0

		// react to emcstat full update message
	} else if *rx.Type == pb.ContainerType_MT_EMCSTAT_FULL_UPDATE {
		if rx.Pparams != nil {
		...
		// INCOMING emcstat incremental update 0 0 0
	} //BBBBBBBB emcstat incremental update
